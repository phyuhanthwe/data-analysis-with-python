...
class BindingSites(unittest.TestCase):
    ...
    def test_euclidean1(self):
        ...
        self.assertAlmostEqual(acc, 0.9865, places=4,
    ...
    def test_euclidean4(self):
        with patch(ph("AgglomerativeClustering"), side_effect=AgglomerativeClustering) as g:
            ...
            self.assertIn('metric', kwargs, msg="You did not give the 'metric' parameter to AgglomerativeClustering!")
            self.assertEqual(kwargs['metric'], "euclidean",
                             "Incorrect argument value of 'metric' passed to AgglomerativeClustering!")

    @points('p06-07.3')
    def test_hamming1(self):
        with patch(ph("plt.show")) as show:
            acc = cluster_hamming("src/data.seq")
            self.assertAlmostEqual(acc, 0.9975, places=4, msg="Function cluster_hamming returned incorrect accuracy for file %s!" % "src/data.seq")
    ...
    def test_hamming4(self):
        with patch(ph("plt.show")) as show:
            with patch(ph("AgglomerativeClustering"), side_effect=AgglomerativeClustering) as g:
                ...
                self.assertIn('metric', kwargs, msg="You did not give the 'metric' parameter to AgglomerativeClustering!")
                self.assertEqual(kwargs['metric'], "precomputed", "Incorrect argument value passed to AgglomerativeClustering!")
                g.assert_called_once()
